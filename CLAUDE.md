...
XiYan-SQL is a natural language to SQL conversion framework that employs a multi-generator ensemble strategy. This repository (**XiYan-SQLTraining**) implements the **training component** for training standard Text-to-SQL models.
...
**This repository trains SQLG₁** - the baseline standard multi-task NL2SQL generator. The other generators (SQLG₂-₄) use SQL formatting variations not yet implemented in this codebase. Schema filtering at inference time and ensemble selection are also separate components not included here.
